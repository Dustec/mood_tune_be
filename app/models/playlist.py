...
__table_args__ = (
CheckConstraint(f"seed_emotion IN {EMOTIONS}", name="ck_playlists_seed_emotion"),
CheckConstraint(f"strategy IN {STRATEGIES}", name="ck_playlists_strategy"),
CheckConstraint(f"provider IN {PROVIDERS}", name="ck_playlists_provider"),
# UNIQUE (provider, provider_playlist_id) cuando provider_playlist_id no es NULL se hará con índice parcial en migración
Index("ix_playlists_user_created_at_desc", "user_id", text("created_at DESC")),
)