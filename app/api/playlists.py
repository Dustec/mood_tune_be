...
from app.schemas.playlist_track import TracksBulkIn
...
def list_playlists(
    ...
    items, next_cursor = PlaylistsService.list(
        db, user_id, provider, limit, cursor)
    return {
...
def finalize_playlist(
    pid: str,
    body: dict | None = Body(default=None, examples=[
                             {"provider_playlist_id": "37i9dQZF1DXcBWIGoYBM5M"}]),
    db: Session = Depends(get_session),
):
